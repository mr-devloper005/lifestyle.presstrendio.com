...
const inputClass = 'h-[3.5rem] rounded-full border border-[#186f65]/15 bg-[#fffdf4] px-5 text-base font-semibold text-[#143c39] outline-none transition placeholder:text-[#42645f]/60 focus:border-[#b2533e] focus:bg-white'
const buttonClass = 'inline-flex h-[3.5rem] items-center justify-center rounded-full bg-[#186f65] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:bg-[#b2533e] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.4rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
