...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-current/20 px-4 py-2 text-xs font-black uppercase tracking-[0.16em]">
...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-[var(--slot4-page-bg)]">
      <header className="premium-hero-curve isolate overflow-hidden bg-[linear-gradient(135deg,var(--slot4-dark-bg),#3d8768_42%,var(--slot4-accent-fill))] text-white">
        <div className="mx-auto max-w-[1240px] px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="rounded-full bg-white/14 px-4 py-2 text-[var(--slot4-accent-soft)]">{categoryOf(post, 'News')}</span>
            {published ? <time className="text-white/78">{published}</time> : null}
          </div>
          <h1 className="editorial-serif mt-6 max-w-6xl text-5xl font-black leading-[0.94] tracking-[-0.055em] sm:text-6xl lg:text-[5.3rem]">{post.title}</h1>
      ...
      {images[0] ? (
        <figure className="mx-auto -mt-16 max-w-[1240px] px-4 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-[2rem] border border-[#186f65]/12 bg-white shadow-[0_26px_70px_rgba(24,111,101,0.16)]">
            <img src={images[0]} alt="" className="max-h-[760px] w-full object-cover" />
            <figcaption className="border-t border-[#186f65]/10 px-4 py-3 text-xs italic text-[var(--slot4-muted-text)] sm:px-6">Featured image for {post.title}</figcaption>
          </div>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1240px] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_340px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[2rem] border border-[#186f65]/12 bg-white p-6 shadow-sm sm:p-8">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="space-y-5">
          <div className="rounded-[2rem] border border-[#186f65]/12 bg-[var(--slot4-panel-bg)] p-6">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent-fill)]">Story context</p>
            <p className="mt-4 text-base leading-8 text-[var(--slot4-muted-text)]">
              Release-ready stories work best when the headline, summary, timing, and supporting details all stay easy to scan.
            </p>
          </div>
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.8rem] border border-[#186f65]/12 bg-white p-6 shadow-[0_30px_90px_rgba(24,111,101,0.09)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[150px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-[2rem] bg-[var(--slot4-panel-bg)] ring-1 ring-[#186f65]/10">
            ...
            <div>
              ...
              <h1 className="mt-3 text-4xl font-black leading-[0.98] tracking-[-0.07em] text-[var(--slot4-dark-bg)] sm:text-6xl">{post.title}</h1>
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-[#186f65]/12 bg-[var(--slot4-dark-bg)] p-7 text-white shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.7rem] border border-[#186f65]/12 bg-white p-6 shadow-[0_30px_90px_rgba(24,111,101,0.08)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.5rem] border border-[#186f65]/12 bg-white p-7 lg:sticky lg:top-24 lg:self-start">
          ...
          <h1 className="mt-6 text-4xl font-black leading-[0.98] tracking-[-0.07em] text-[var(--slot4-dark-bg)] sm:text-5xl">{post.title}</h1>
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-[#186f65]/12 bg-white shadow-sm">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-[#186f65]/12 bg-white p-7 shadow-[0_30px_90px_rgba(24,111,101,0.08)] sm:p-10">
        <BackLink task="sbm" />
        ...
        <h1 className="mt-7 text-4xl font-black leading-[0.98] tracking-[-0.07em] text-[var(--slot4-dark-bg)] sm:text-6xl">{post.title}</h1>
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-[#186f65]/12 bg-white p-6 shadow-[0_30px_90px_rgba(24,111,101,0.08)] sm:p-9">
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-[#186f65]/12 bg-[var(--slot4-panel-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-[#186f65]/12 bg-white p-4">
              <span className="text-sm font-black text-[var(--slot4-dark-bg)]">Document preview</span>
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.7rem] border border-[#186f65]/12 bg-white p-8 text-center shadow-[0_30px_90px_rgba(24,111,101,0.08)] lg:sticky lg:top-24 lg:self-start">
        <BackLink task="profile" />
        <div className="mx-auto mt-10 flex h-40 w-40 items-center justify-center overflow-hidden rounded-full bg-[var(--slot4-panel-bg)] ring-1 ring-[#186f65]/10">
          {images[0] ? <img src={images[0]} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-16 w-16 opacity-45" />}
        </div>
        <h1 className="mt-6 text-4xl font-black leading-[0.98] tracking-[-0.07em] text-[var(--slot4-dark-bg)]">{post.title}</h1>
        {role ? <p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-[var(--slot4-accent-fill)]">{role}</p> : null}
        <ContactAction website={website} email={email} />
      </aside>
      <article className="rounded-[2.7rem] border border-[#186f65]/12 bg-white p-7 shadow-sm sm:p-10">
        <BodyContent post={post} />
  ...
  return (
    <div className="mt-8 grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="rounded-[1.5rem] border border-[#186f65]/12 bg-[var(--slot4-panel-bg)] p-4">
  ...
  return (
    <section className="mt-8">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] rounded-[1.4rem] object-cover ring-1 ring-[#186f65]/12" />)}
      </div>
    </section>
  )
}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-[#186f65]/12 bg-white shadow-sm">
      <div className="flex items-center gap-2 p-4 text-sm font-black text-[var(--slot4-dark-bg)]"><MapPin className="h-4 w-4" /> {label || 'Map location'}</div>
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-[#186f65]/12 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-full border border-[#186f65]/12 px-4 py-2 text-sm font-black text-[var(--slot4-page-text)]"><Phone className="h-4 w-4" /> Call</a> : null}
        {email ? <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-full border border-[#186f65]/12 px-4 py-2 text-sm font-black text-[var(--slot4-page-text)]"><Mail className="h-4 w-4" /> Email</a> : null}
      </div>
    </div>
  )
}
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-[#186f65]/12 bg-white p-5 shadow-sm">
      ...
      {related.length ? (
        <div className="rounded-[2rem] border border-[#186f65]/12 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-black tracking-[-0.04em] text-[var(--slot4-dark-bg)]">More like this</h2>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#186f65]/10 py-3 transition hover:text-[var(--slot4-accent-fill)]">
      ...
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-black leading-tight tracking-[-0.03em] text-[var(--slot4-dark-bg)]">{post.title}</h3>
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[2rem] bg-[var(--slot4-panel-bg)] p-5">
      ...
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-[1.4rem] bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm font-black text-[var(--slot4-dark-bg)]">{comment.name}</p>
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--slot4-muted-text)]">
        ...
        {!comments.length ? <p className="rounded-[1.4rem] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">No comments yet for {slug}.</p> : null}
      </div>
    </section>
  )
}
